...
export async function initDb() {
  ...
  try { await db.prepare("ALTER TABLE umrah_packages ADD COLUMN makkah_hotel_distance TEXT").run(); } catch {}
  ...
  try { await db.prepare("ALTER TABLE umrah_packages DROP COLUMN visa_price").run(); } catch {}
  try { await db.prepare("ALTER TABLE umrah_packages DROP COLUMN arrival_date").run(); } catch {}
  try { await db.prepare("ALTER TABLE umrah_packages DROP COLUMN distance_meters").run(); } catch {}
  try { await db.prepare("ALTER TABLE umrah_packages DROP COLUMN hotel_distance").run(); } catch {}
}
...
export async function seedDb() {
  ...
  if (!pkgCount || pkgCount.count === 0) {
    const packages = [
      ['Umrah by Air - Saudia', 'Saudia', '2026-07-15', '2026-07-25', 10, 145000, 'Swissotel Makkah', 'Movenpick Madina', 'https://images.unsplash.com/photo-1565058688641-6776481d1b84?w=800&auto=format&fit=crop', 145000, 175000, 155000, 145000, 'Lahore', 'Jeddah', 45, '50 Meters', '80 Meters', 6, 3],
      ['Umrah by Air - PIA', 'PIA', '2026-07-20', '2026-07-30', 10, 135000, 'Hilton Makkah', 'Pullman Zamzam', 'https://images.unsplash.com/photo-1564769625905-50e93615e769?w=800&auto=format&fit=crop', 135000, 165000, 145000, 135000, 'Karachi', 'Jeddah', 40, '100 Meters', '120 Meters', 6, 3],
      ['Umrah by Air - Airblue', 'Airblue', '2026-08-05', '2026-08-15', 10, 125000, 'Makkah Clock Royal', 'Movenpick Anwar', 'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=800&auto=format&fit=crop', 125000, 155000, 135000, 125000, 'Islamabad', 'Jeddah', 50, '200 Meters', '250 Meters', 6, 3],
      ['Umrah by Air - SereneAir', 'SereneAir', '2026-08-10', '2026-08-20', 10, 120000, 'Raffles Makkah', 'Dar Al Taqwa', 'https://images.unsplash.com/photo-1527612820672-5b56351f7346?w=800&auto=format&fit=crop', 120000, 150000, 130000, 120000, 'Lahore', 'Jeddah', 55, '150 Meters', '180 Meters', 6, 3],
      ['Umrah Package July', 'Saudia', '2026-07-01', '2026-07-12', 11, 155000, 'Conrad Makkah', 'Shaza Madina', 'https://images.unsplash.com/photo-1584551246679-0daf3d275d0f?w=800&auto=format&fit=crop', 155000, 185000, 165000, 155000, 'Lahore', 'Madina', 30, '300 Meters', '350 Meters', 7, 3],
      ['Umrah Package August', 'PIA', '2026-08-01', '2026-08-12', 11, 140000, 'Hyatt Regency', 'Marriott Madina', 'https://images.unsplash.com/photo-1548685913-fe6678babe8d?w=800&auto=format&fit=crop', 140000, 170000, 150000, 140000, 'Karachi', 'Madina', 35, '250 Meters', '280 Meters', 7, 3],
    ];
    for (const p of packages) {
      await db.prepare(`INSERT INTO umrah_packages (title, airline, departure_date, return_date, days, price, hotel_makkah, hotel_madina, image_url, sharing_price, double_price, triple_price, quad_price, from_city, to_city, seats, makkah_hotel_distance, madina_hotel_distance, makkah_nights, madina_nights) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(...p);
    }
  }
