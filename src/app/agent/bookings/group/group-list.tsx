...
export default function GroupBookingClient({
  ...
  const [selectedGroup, setSelectedGroup] = useState<OneWayGroup | null>(null);
  ...
  }, [groups, selectedAirlines, minPrice, maxPrice]);
  ...
  const openBookModal = (g: OneWayGroup) => {
    setSelectedGroup(g);
    setAdults(1);
    setInfants(0);
    setBookingError("");
  ...
  const handleBooking = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const bookingsRes = await fetch("/api/agent/bookings?type=group");
      if (bookingsRes.ok) {
  ...
  const hasActiveFilters =
    selectedAirlines.length > 0 || minPrice || maxPrice;

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-[#0c1d4a]">One-way Groups</h1>
        ...
        <div className="flex-1">
          {filteredGroups.length === 0 ? (
            <div className="bg-white rounded-lg shadow-sm border p-8 text-center text-gray-500">
              No one-way groups match your filters.
            </div>
          ) : (
          ...
          <div className="mt-8 bg-white rounded-lg shadow-sm border">
            <div className="px-6 py-4 border-b">
              <h2 className="text-lg font-bold">My Group Bookings</h2>
            </div>
            <div className="p-6">
              {bookings.length === 0 ? (
      ...
      <Dialog open={!!selectedGroup} onOpenChange={(open) => !open && closeModal()}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Book Group</DialogTitle>
          </DialogHeader>
            ...
            <form onSubmit={handleBooking} className="space-y-4">
              <div className="bg-gray-50 rounded-md p-3 text-sm">
                ...
                <p className="text-gray-500 text-xs mt-1">
                  {selectedGroup.airline || "N/A"} · {selectedGroup.destination} · {selectedGroup.departure_date}
                </p>
              </div>
